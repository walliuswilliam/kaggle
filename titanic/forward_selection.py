...
def fit_regressor(df, iters=1000):
  df_train = df[:500]
  df_test = df[500:]
  ...
  regressor = LogisticRegression(max_iter=iters)
...
df = pd.read_csv('titanic/csv/interactions.csv')
...
def next_best_feature(input_features):
  original_list = input_features.copy()
  original_accuracy = fit_regressor(df[input_features])
  temp_accuracy = 0
  accuracy = 0
  best_feature = None
  for feature in features:
    if feature not in input_features:
      ...
      temp_df = df[input_features]
      temp_accuracy = fit_regressor(temp_df)
      if temp_accuracy == accuracy:
        input_features.remove(feature)
      elif temp_accuracy > accuracy:
  ...
  if accuracy != original_accuracy:
    original_list.append(best_feature)
    return original_list, accuracy

past_iteration = []
current_iteration = ['Survived', 'Sex']
for num in range(10):
  print(next_best_feature(current_iteration)[0])
  print(next_best_feature(current_iteration)[1], '\n')
  past_iteration = current_iteration
  current_iteration = next_best_feature(current_iteration)[0]
